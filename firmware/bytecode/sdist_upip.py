...
def make_resource_module(manifest_files):
        ...
        for fname in manifest_files:
            ...
            if ext not in ['py', 'mpy']:
                resources.append(fname)

            else:
                if fname.endswith('__init__.mpy'):
                    # automatically add an empty __init__.py which is required by distutils
                    with open(fname.replace('__init__.mpy', '__init__.py'), 'w'):
                        pass
